...
class IPKernelServer(BaseIPythonApplication, InteractiveShellApp,
        ConnectionFileMixin):
    name='ipython-kernel'
    ...
    def initialize(self, argv=None):
        super(IPKernelServer, self).initialize(argv)
...
launch_new_instance = IPKernelServer.launch_instance
...
def loop_slicer(kernel):
    ...
    class Timer(object):
        ...
        def on_timer(self):
            self.timer.interval = poll_interval
            self.timer.connect('timeout()', self.func)
            self.timer.start()
...
def main():
    ...
    global app
    app = IPKernelServer.instance()
    app.initialize()
    app.kernel.start()
    loop_slicer(app.kernel)
    slicer.app.connect('aboutToQuit()', app.kernel.timer.timer.stop) 
    #app.start()
